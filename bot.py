...
CATEGORY_CHANNELS = {
    ...
    "General" : "channel id" ,
    "On-site" : "channel id" ,
    "Site Feedback" : "channel id" ,
    "welcome channel" : "channel id"  
}

CATEGORY_MENTIONS = {
   "Announcements" : ["&role id"],
    "General" : ["&role id"] # Replace with actual role IDs
    
}


FORUM_URL = "https://erc2025.husarion.com/" 
...
intents.guilds = True 
intents.members = True  # manage members
        ...
        try:
            ...
            for a_tag in topic_links:
                ...
                if channel_id:
                    ...
                    if channel:
                        mentions = CATEGORY_MENTIONS.get(category_name, [])
                        mention_text = " ".join(f"<@&{uid[1:]}>"  if uid.startswith("&") else f"<@{uid}>" for uid in mentions)

                        #message = f"{mention_text}\n**{title}**\n{full_link}" if mention_text else f"**{title}**\n{full_link}"
